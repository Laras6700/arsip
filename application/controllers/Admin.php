...
class Admin extends CI_Controller
{
	...
	public function aksibalassm()
	{
		$id_suratmasuk = $this->input->post('id_suratmasuk');
		$no_suratmasuk = htmlspecialchars($this->input->post('no_suratmasuk'));
		$judul_suratmasuk = htmlspecialchars($this->input->post('judmul_suratmasuk'));
		$asal_surat = htmlspecialchars($this->input->post('asal_surat'));
		$namaberkas_suratmasuk = $_FILES['berkas_suratmasuk']['name'];
		$exp = explode('.', $namaberkas_suratmasuk);
		$typeberkas_suratmasuk = end($exp);
	...
	public function suratkeluar()
	{
		...
		$this->db->join('indeks', 'id_indeks', 'left');
		$this->db->join('suratmasuk', 'id_suratmasuk', 'left');
	...
	public function jadwal()
	{
		...
		// Jika form tambah jadwal dikirimkan
		if ($this->input->post('tambah')) {
			...
			// Menambahkan data jadwal menggunakan model
			$this->Model_jadwal->tambahJadwal($jadwal_data);

			// Mengarahkan kembali ke halaman jadwal
			redirect('admin/jadwal');
		}

		// Jika form edit jadwal dikirimkan
		if ($this->input->post('edit')) {
			...
			// ID jadwal yang akan diupdate
			$jadwal_id = $this->input->post('jadwal_id');

			// Mengupdate data jadwal menggunakan model
			$this->Model_jadwal->updateJadwal($jadwal_id, $jadwal_data);

			// Mengarahkan kembali ke halaman jadwal
			redirect('admin/jadwal');
		}

		// Jika form hapus jadwal dikirimkan
		if ($this->input->post('hapus')) {
			// ID jadwal yang akan dihapus
			$jadwal_id = $this->input->post('jadwal_id');

			// Menghapus data jadwal menggunakan model
			$this->Model_jadwal->hapusJadwal($jadwal_id);

			// Mengarahkan kembali ke halaman jadwal
			redirect('admin/jadwal');
		}

		// Mengambil data jadwal menggunakan model
		$data['jadwal'] = $this->Model_jadwal->getJadwal();
